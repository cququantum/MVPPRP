...
\begin{remark}[RF列生成的Phase-I实现细节（避免初始RMP不可行）]
在实现\eqref{eq:rf_lp}时，受限主问题（RMP）可能因初始列不足而暂时不可行（例如仅含单客户路线时，无法同时满足全部覆盖下界）。
推荐采用Phase-I机制区分“初始RMP不可行”和“RF模型本身不可行”：
\begin{itemize}
  \item 仅在覆盖下界约束 $\sum_{r\in\mathcal R} a_{ir}\zeta_r \ge f_{il}$ 上添加人工变量 $a_i\ge 0$，并在目标中加入大罚项 $M\sum_i a_i$；
  \item 路线数上下界约束 $\sum_r\zeta_r\ge m_L,\ \sum_r\zeta_r\le m_U$ 不添加人工变量；
  \item 当列生成收敛后，若仍存在 $a_i>0$，则判定RF确实不可行；若全部 $a_i=0$，则当前目标值即为有效的 $LB_R$；
  \item $M$ 需显著大于正常路线成本量级（即远大于典型 $b_r$），以避免人工变量在最优解中残留。
\end{itemize}
若RF最终被判定不可行，实现上可回退到保守下界（如$LB_R=0$），不影响割的有效性，但会削弱组合最优性割强度。
\end{remark}
