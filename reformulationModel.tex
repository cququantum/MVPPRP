...
\begin{remark}[虚拟汇节点 $l+1$ 的实现注意事项]
约束 (2-42) 将 $z_{it}$ 与 $\lambda$ 链接，覆盖范围为 $t\in T$（不含 $l+1$）。而约束 (2-44) 中的 "终止弧" $\lambda_{it,l+1}$ 仅作为最短路网络的汇点，表示 "上次访问为 $t$、网络在 $l+1$ 结束"，并不对应实际的第 $l+1$ 期访问行为。具体而言：
\begin{itemize}
  \item $z_{i,l+1}$ 未被定义（$l+1\notin T$），实现时不应为其分配变量或产生路由成本；
  \item $\lambda_{it,l+1}$ 的取货量 $g_{it,l+1}$ 及库存成本 $e_{it,l+1}$ 按正常公式计算（代表 $t$ 期到规划期末的剩余积累量），但在代码中需与 $t\in T$ 的弧严格区分，避免将其误输入车辆路径子问题（BSP）作为实际取货任务。
\end{itemize}
\end{remark}
