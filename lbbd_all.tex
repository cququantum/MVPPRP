...
\begin{equation}
\sum_{i\in N_c^t(\bar z)}\ \sum_{v=\pi(i,t)}^{\bar v_i}\Bigl(1-\lambda_{ivt}\Bigr)\ \ge\ 1,
\qquad \forall t\in T.
\label{eq:feas_316}
\end{equation}
...
\begin{equation}
\omega_t\ \ge\ \sum_{i\in N_c^t(\bar z)}\ \sum_{v=\pi(i,t)}^{\bar v_i} u_i^*\,\lambda_{ivt}\ +\ K u_0^*,
\qquad \forall t\in T.
\label{eq:opt_319}
\end{equation}
...
\paragraph{F.2 松弛路由问题（RF）（式(3-24)）}
...
\paragraph{F.3 $LB_R$定义（实现直接用）}
\begin{equation}
LB_R\ :=\ z_{LP}(RF),
\label{eq:lbr_value}
\end{equation}
其中$z_{LP}(RF)$表示将\eqref{eq:rf_324}中的$\zeta_r\in\mathbb Z$放宽为连续变量$\zeta_r\ge 0$后的LP最优值。
